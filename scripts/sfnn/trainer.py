...
def make(config, task, params_like)->EvosaxTrainer:
	...
	def metrics_fn(state, data):
		...
		y['cartpole_mean_fit'] = data['cart_fit'].mean()
		y['acrobot_mean_fit'] = data['acro_fit'].mean()
		y['mountaincar_mean_fit'] = data['mount_fit'].mean()
		return y, (state.mean, state.best_member), state.gen_counter
